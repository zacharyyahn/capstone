...
void TA0_N_IRQHandler() {
    ...
    if (TIMER_A0->CCTL[LDEF_CCR_INDEX] & TIMER_A_CCTLN_CCIFG) LINEAR_CONTROL_PORT->OUT &= ~LDef_PWM_Pin;
    if (TIMER_A0->CCTL[LOFF_CCR_INDEX] & TIMER_A_CCTLN_CCIFG) LINEAR_CONTROL_PORT->OUT &= ~LOff_PWM_Pin;
    if (TIMER_A0->CCTL[RDEF_CCR_INDEX] & TIMER_A_CCTLN_CCIFG) ROTATIONAL_CONTROL_PORT->OUT &= ~RDef_PWM_Pin;
    if (TIMER_A0->CCTL[ROFF_CCR_INDEX] & TIMER_A_CCTLN_CCIFG) ROTATIONAL_CONTROL_PORT->OUT &= ~ROff_PWM_Pin;

    // Assuming IRQ takes less than 1 timer count to complete, all flags can be reset
